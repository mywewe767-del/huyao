...
import type { CanvasTool, CrossingMode, DirectionLayer, PatternDefinition, PatternInstance, Point, RegionAdjacency, TransitionZone, WeaveDocument, WeaveRegion } from "../types/weave";

const STORAGE_KEY = "bamboo-weave-studio:v3";
...
function patternInstance(patternId: string, customPatterns: PatternDefinition[] = []): PatternInstance {
  ...
  return { patternId, scale: 1, scaleX: 1, scaleY: 1, rotation: 0, density: 100, stripWidth: pattern.defaultStripWidth,
    directionLayers: structuredClone(pattern.directionLayers), crossingRules: structuredClone(pattern.crossingRules), crossingOverrides: {}, directionWidths: {}, directionColors: {}, localOverrides: {} };
}
...
export function readAutosave(): WeaveDocument | null {
  ...
  try {
    const raw = localStorage.getItem(STORAGE_KEY) ?? localStorage.getItem("bamboo-weave-studio:v2"); if (!raw) return null;
    ...
    return { ...legacy, version: 3, name: legacy.name.replace(/V2/g, "V3"), customPatterns, regions: legacy.regions.map((item) => { const defaults = patternInstance(item.pattern.patternId, customPatterns); return { ...item, pattern: { ...defaults, ...item.pattern, scaleX: item.pattern.scaleX ?? item.pattern.scale ?? 1, scaleY: item.pattern.scaleY ?? item.pattern.scale ?? 1, directionLayers: item.pattern.directionLayers?.length ? item.pattern.directionLayers : defaults.directionLayers, crossingRules: item.pattern.crossingRules?.length ? item.pattern.crossingRules : defaults.crossingRules, crossingOverrides: item.pattern.crossingOverrides ?? {} } }; }), transitions: legacy.transitions.map((item) => ({ ...item, strategy: item.strategy ?? "spacing-gradient", stageCount: item.stageCount ?? 12, bridgePatternIds: item.bridgePatternIds ?? [], regenerationSeed: item.regenerationSeed ?? 0, fanStrength: item.fanStrength ?? 45, spreadAngle: item.spreadAngle ?? 35, curves: item.curves ?? [] })) };
  } catch { return null; }
}
