...
- 50 步 Undo/Redo、LocalStorage 自动保存、V3 JSON 读取/保存与 PNG 导出
...
7. 选择过渡区后替换策略，调整宽度、阶段、平滑度和复杂度，或删除/重新生成。
8. 点击“从图片提取纹样”生成并校正新的结构纹样。
9. 切换 `3D Realistic` 检查实体条带和过渡曲线。

## 架构

- `src/types`：V3 Document、Direction Layer、Crossing Rule、Pattern Instance、Transition Strategy 模型
